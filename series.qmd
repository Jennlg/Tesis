...
::: {#thm-varmedia}
Sea $X_t$ una serie de tiempo estacionaria, la varianza de $\bar X$ basada en una realización de longitud $n$ está dada por
...
::: {style="text-align: justify"}
El resultado en la ecuación ([-@eq-varmedia]) muestra el efecto de la autocorrelación en la varianza de $\bar X$, y si $X_t$ es ruido blanco, es decir, $\gamma_{_{k}} = 0$ si $k \ne 0$, entonces la ecuación ([-@eq-varmedia]) se convierte en el conocido resultado $Var(\bar X) = \sigma^2/n$.
...
::: {.remark style="text-align: justify"}
Usando la ecuación ([-@eq-estgamma]) se deduce que
...
::: {style="text-align: justify"}
A partir del análisis de la ecuación ([-@eq-estgamma]) , es evidente que los valores de $\hat\gamma_{_{k}}$ (y $\hat\rho_{_{k}}$) tenderán a ser "pequeños" cuando $k$ sea grande en relación con $n$.
:::
...
Usando la fórmula de Euler en las ecuaciones ([-@eq-spectrum]) y ([-@eq-spectden]), se obtienen las siguientes fórmulas.
...
Estas fórmulas enfatizan que el espectro y la densidad espectral son funciones de valor real, lo que no es evidente en las ecuaciones ([-@eq-spectrum]) y ([-@eq-spectden]).
...
Los datos $x_{_t}$, en el tiempo $t$ pueden ser considerados como una suma dada en la ecuación ([-@eq-addseas])
...
Los datos, $x_{_t}$, en el tiempo $t$ pueden ser expresados como el producto dado en la ecuación ([-@eq-mulseas])
...
Los datos logarítmicos de logAirPassengers en la @fig-air-2 no muestran un aumento en la variabilidad dentro del año y son un ejemplo clásico de datos que se modelan utilizando el modelo aditivo en la ecuación ([-@eq-addseas]).
:::
:::
...
::: {style="text-align: justify"}
Cuando se analizan datos utilizando el modelo aditivo en la ecuación ([-@eq-addseas]), se parte del supuesto de que los datos son la suma de componentes estacionales, de tendencia y de ruido aleatorio. Se discuten los pasos de análisis involucrados en la descomposición de los datos logarítmicos de logAirPassengers. En la práctica, los componentes en la ecuación ([-@eq-addseas]) se estiman y un modelo estimado puede describirse como
...
a.  ***Estimar el Componente de Tendencia***: La @fig-sm12 es una representación gráfica del conjunto de datos logAirPassengers superpuesto con el resultado de aplicar un suavizador de media móvil centrada de orden 12 a los datos.
    ...
    En relación con el modelo estimado en la ecuación ([-@eq-addecompose]), $\hat{tr_{_t}} =$ **logair.sm12** representa la curva casi lineal en la @fig-sm12.
...
c.  ***Calcular un "Promedio" del Componente Estacional Dentro del Año:*** Es importante notar que, el componente estacional en el modelo (ecuación ([-@eq-addseas])), es un patrón general que se mantiene igual de un año a otro. Es decir,$\{s_{_t}, t=1,\ldots,12\} = \{s_{_{t+12}},t=1,\ldots,12\} =\{s_{_{t+2(12)}}, t=1,\ldots,12\}=\cdots.$ El componente de ruido, $z_{_t}$, ajusta las variaciones de un año a otro del patrón estacional general. El patrón estacional, $s_{_t}, t=1,\ldots,12$, se estima calculando el promedio a lo largo de los años y, el componente estacional estimado, $\hat{s}_{_t}$ (el cual es el mismo para cada año), se muestra en la @fig-seasair-2.
...
d.  ***Encontrar el componente de ruido restante:*** El ruido estimado en la ecuación ([-@eq-addecompose]), $\hat{z_{_t}}$, es calculado de la siguiente manera
...
a.  ***Estimar el Componente de Tendencia***: La @fig-sm12p es una representación gráfica del conjunto de datos logAirPassengers superpuesto con el resultado de aplicar un suavizador de media móvil centrada de orden 12 a los datos.
    ...
    ``` r
      [1]       NA       NA       NA       NA       NA       NA
      [7] 4.837280 4.841114 4.846596 4.851238 4.854488 4.859954
     [13] 4.869840 4.881389 4.893411 4.904293 4.912752 4.923701
     [19] 4.940483 4.957406 4.974380 4.991942 5.013095 5.033804
     [25] 5.047776 5.060902 5.073812 5.088378 5.106906 5.124312
     [31] 5.138282 5.152751 5.163718 5.171454 5.178401 5.189431
     [37] 5.203909 5.218093 5.231553 5.243722 5.257413 5.270736
     [43] 5.282916 5.292150 5.304079 5.323338 5.343560 5.357427
     [49] 5.367695 5.378309 5.388417 5.397805 5.403849 5.407220
     [55] 5.410364 5.410294 5.408381 5.406761 5.406218 5.410571
     [61] 5.419628 5.428330 5.435128 5.442237 5.450659 5.461103
     [67] 5.473655 5.489713 5.503974 5.516367 5.529403 5.542725
     [73] 5.557864 5.572693 5.587498 5.602730 5.616658 5.631189
     [79] 5.645937 5.659812 5.674172 5.687636 5.700766 5.714738
     [85] 5.727153 5.738856 5.750676 5.760658 5.770846 5.780430
     [91] 5.788745 5.796524 5.804821 5.814072 5.823075 5.832692
     [97] 5.842665 5.853541 5.864863 5.875490 5.885654 5.894475
    [103] 5.901555 5.907026 5.910012 5.910708 5.911637 5.913829
    [109] 5.917360 5.922887 5.926146 5.927563 5.929657 5.930458
    [115] 5.932964 5.938377 5.946188 5.956352 5.967813 5.977291
    [121] 5.985269 5.994078 6.003991 6.014899 6.026589 6.040709
    [127] 6.054492 6.066195 6.073088 6.080733 6.091930 6.102013
    [133] 6.112511 6.121153 6.128381 6.137437 6.145733 6.151526
    [139]       NA       NA       NA       NA       NA       NA
    ```

    En relación con el modelo estimado en la ecuación ([-@eq-addecompose]), $\hat{tr_{_t}} =$ **logair.sm12** representa la curva casi lineal en la @fig-sm12p.
...
b.  ***Eliminar el Componente de Tendencia de los Datos:*** El paso subsiguiente implica la sustracción del componente de tendencia estimado de los datos (logAirPassengers).
    ...
    1949      NA      NA      NA      NA      NA      NA  0.1599
    1950 -0.1249 -0.0451  0.0553  0.0010 -0.0844  0.0802  0.1953
    1951 -0.0710 -0.0503  0.1080  0.0054  0.0406  0.0575  0.1550
    1952 -0.0622 -0.0251  0.0311 -0.0452 -0.0479  0.1138  0.1552
    1953 -0.0896 -0.1002  0.0754  0.0618  0.0299  0.0858  0.1656
    1954 -0.1015 -0.1919  0.0245 -0.0173  0.0047  0.1148  0.2368
    1955 -0.0689 -0.1217 -0.0002 -0.0080 -0.0182  0.1214  0.2512
    1956 -0.0782 -0.1148  0.0082 -0.0145 -0.0088  0.1438  0.2347
    1957 -0.0901 -0.1464  0.0101 -0.0233 -0.0135  0.1505  0.2405
    1958 -0.0884 -0.1608 -0.0345 -0.0754 -0.0353  0.1449  0.2635
    1959 -0.0992 -0.1593  0.0024 -0.0335  0.0137  0.1163  0.2518
    1960 -0.0794 -0.1524 -0.0905 -0.0040  0.0112  0.1307      NA
             Aug     Sep     Oct     Nov     Dec
    1949  0.1561  0.0661 -0.0721 -0.2101 -0.0893
    1950  0.1784  0.0882 -0.1016 -0.2769 -0.0922
    1951  0.1406  0.0512 -0.0839 -0.1948 -0.0774
    1952  0.1968  0.0383 -0.0711 -0.1961 -0.0896
    1953  0.1955  0.0597 -0.0549 -0.2133 -0.1073
    1954  0.1905  0.0529 -0.0826 -0.2162 -0.1090
    1955  0.1895  0.0688 -0.0745 -0.2327 -0.0871
    1956  0.2074  0.0673 -0.0905 -0.2210 -0.1091
    1957  0.2393  0.0914 -0.0614 -0.1913 -0.0967
    1958  0.2862  0.0552 -0.0730 -0.2312 -0.1572
    1959  0.2600  0.0646 -0.0719 -0.2003 -0.0981
    1960      NA      NA      NA      NA      NA
    ```
...
Se aprecia con mayor claridad el comportamiento estacional año tras año en la @fig-seasairp-1 sin la "interferencia" de la tendencia. Específicamente, se observa un patrón similar en cada año (mayor cantidad de viajes en verano, disminución en noviembre, aún bajos pero con una ligera alza en diciembre, continuamente bajos en enero y febrero, alza en marzo, y así sucesivamente). No obstante, se presentan variaciones de un año a otro: los viajes aéreos en noviembre fueron notablemente bajos en 1950 y luego inusualmente altos en julio y agosto de 1958.

c.  ***Calcular un "Promedio" del Componente Estacional Dentro del Año:*** Es importante notar que, el componente estacional en el modelo (ecuación ([-@eq-addseas])), es un patrón general que se mantiene igual de un año a otro. Es decir,$\{s_{_t}, t=1,\ldots,12\} = \{s_{_{t+12}},t=1,\ldots,12\} =\{s_{_{t+2(12)}}, t=1,\ldots,12\}=\cdots.$ El componente de ruido, $z_{_t}$, ajusta las variaciones de un año a otro del patrón estacional general. El patrón estacional, $s_{_t}, t=1,\ldots,12$, se estima calculando el promedio a lo largo de los años y, el componente estacional estimado, $\hat{s}_{_t}$ (el cual es el mismo para cada año), se muestra en la @fig-seasairp-2.
    ...
    ``` r
    [1] -0.0867 -0.1153  0.0172 -0.0139 -0.0098  0.1145  0.2100
    [8]  0.2036  0.0640 -0.0761 -0.2167 -0.1012
    ```
...
La @fig-descaddp muestra un gráfico de los datos de LogAirPassengers junto con las partes del procedimiento de descomposición.
:::
:::
:::
:::
...
Se llevará a cabo en el @exm-mulairp una descomposición multiplicativa de los datos de AirPassengers. Se destaca que esta serie temporal exhibe un patrón estacional y una variabilidad intra-anual crecientes con el tiempo. A pesar de la posibilidad de modelar estos datos mediante el uso del logaritmo seguido de un modelo aditivo, en esta sección se opta por un enfoque multiplicativo para analizar los datos originales de AirPassengers. Al emplear la descomposición multiplicativa en el análisis de datos, se hace la suposición de que la serie temporal es el resultado de componentes estacionales, de tendencia y de ruido. El modelo estimado se expresa como;
...
a.  ***Estimar el Componente de Tendencia***: Al igual que con el modelo aditivo, el primer paso consiste en utilizar un suavizador de media móvil centrada, nuevamente en este caso de orden 12. Anteriormente se calculó y representó gráficamente el suavizador de media móvil en la @fig-smapp.
    ...
    Es importante recordar que, en relación con el modelo estimado en la ecuación ([-@eq-muldecomp]), $\hat{tr_{_t}} =$ **AirPass.sm12**. Esta curva casi lineal se muestra como parte de la descomposición completa en la @fig-descmulp.
...
b.  ***Eliminar el Componente de Tendencia de los Datos:*** El siguiente paso consiste en eliminar el componente de tendencia estimado del conjunto de datos de AirPassengers. Esto se puede lograr mediante la división (en lugar de la resta).
    ...
    La conducta estacional de un año a otro resulta mucho más clara en la @fig-seasmulp-1 después de eliminar la "interferencia" de la tendencia y el aumento de la variabilidad dentro del año. También se observa que la variabilidad dentro del año no está aumentando tanto como en la @fig-airpass. El incremento en la variabilidad en el modelo final (ecuación ([-@eq-muldecomp])) se debe a la tendencia creciente que se multiplica por los datos estacionales en la @fig-seasmulp-1. Los patrones estacionales en la @fig-seasmulp-1 son similares a los de los datos aditivos mostrados en la @fig-seasairp-1.
...
d.  ***Encontrar el componente de ruido restante:*** El ruido estimado en la ecuación ([-@eq-muldecomp]), $\hat{z_{_t}}$, es calculado de la siguiente manera
...
La @fig-descmulp muestra un gráfico de los datos de AirPassengers junto con las partes del procedimiento de descomposición.
:::
...
$$ x_{_t}=\hat{s_{_t}}\times \hat{tr_{_t}}\times \hat{z_{_t}}. $$ {#eq-muldecom}
...
a.  ***Estimar el Componente de Tendencia***: Al igual que con el modelo aditivo, el primer paso consiste en utilizar un suavizador de media móvil centrada, nuevamente en este caso de orden 12. Anteriormente se calculó y representó gráficamente el suavizador de media móvil en la @fig-smap.
    ...
    Es importante recordar que, en relación con el modelo estimado en la ecuación ([-@eq-muldecom]), $\hat{tr_{_t}} =$ **AirPass.sm12**. Esta curva casi lineal se muestra como parte de la descomposición completa en la @fig-descmul.
...
d.  ***Encontrar el componente de ruido restante:*** El ruido estimado en la ecuación ([-@eq-muldecom]), $\hat{z_{_t}}$, es calculado de la siguiente manera
...
::: {.content-visible when-format="pdf" style="text-align: justify"}
que resta el componente estacional (mostrado en la @fig-seasairp-2) de los datos.
:::
...
::: {.content-visible when-format="pdf" style="text-align: justify"}
para dividir los datos por el componente estacional (mostrado en la @fig-seasmulp-2).
:::
...
La @fig-AEmul-1 muestra los datos de Pasajeros Aéreos superpuestos con la estimación de tendencia obtenida utilizando un suavizador de media móvil centrada de orden 12. La @fig-AEmul-2 es una representación gráfica de los datos ajustados estacionalmente calculados utilizando la ecuación ([-@eq-AEMul]). Es decir, es una representación de **AirPassengers.adj**. Una vez más, los datos ajustados estacionalmente son similares a la tendencia estimada pero con más detalle respecto a los cambios mensuales.
...
-   Si se usa un predictor de promedio móvil de 3 puntos para un conjunto de datos de longitud $t=20$, se observa que la predicción un paso hacia adelante de $x_4$ usando este predictor sería
...
Es claro a partir de la ecuación ([-@eq-fexp]) que como $\alpha$ está entre cero y uno, la fórmula recursiva otorga menos peso a las observaciones anteriores y más peso a las observaciones más recientes, y que la magnitud del ponderado es exponencial.
:::
...
La @fig-PAP-3 muestra los valores $u_{_t}$ y exhibe pronósticos a un paso utilizando suavizado exponencial con $\alpha = 0.4$. Los predictores resultantes son similares a los de la @fig-PAP-2.
...
::: {style="text-align: justify"}
El enfoque Holt-Winters es una técnica desarrollada por los economistas @holt1957forecasting y @winters1960forecasting.
...
::: {style="text-align: justify"}
El modelo aditivo de Holt-Winters para datos con tendencia y estacionalidad que no aumentan con el tiempo es adecuado (consulte la ecuación ([-@eq-addseas])). Las fórmulas para la predicción de Holt-Winters son generalizaciones de las ecuaciones de suavizado exponencial; y las ecuaciones de Holt-Winters son las siguientes:
...
::: {style="text-align: justify"}
Como sugiere el término, las ecuaciones multiplicativas de Holt-Winters son aplicables a datos para los cuales el modelo multiplicativo (ecuación ([-@eq-mulseas])) es apropiado. En este caso, las ecuaciones de Holt-Winters son:
...
::: {style="text-align: justify"}
La fórmula en la ecuación ([-@eq-ARp]) indica que, el valor del proceso en el tiempo $t$, es una combinación lineal de los $p$ valores anteriores más un componente de ruido aleatorio en $a_{_t}$. Iniciamos nuestra discusión sobre los modelos $\mathrm{AR}$ al abordar sus propiedades, incluidas las condiciones de estacionariedad y el comportamiento de las autocorrelaciones y densidades espectrales para modelos específicos.

El modelo $\mathrm{AR}(p)$ general definido en la @def-mARp, se asemeja a una ecuación de regresión múltiple, donde, en este caso, las "variables independientes" son los $p$ valores previos de la "variable dependiente" $X_{_t}$. Otra forma de escribir la ecuación ([-@eq-ARp]), después de reorganizar los términos, es:
...
iii. La autocorrelación de un proceso $\mathrm{AR}(p)$ satisface $$
     ...
     La ecuación ([-@eq-autoarp]) conduce a las ecuaciones de Yule-Walker de orden $p\times p$: $$
         \begin{split}
...
::: {style="text-align: justify"}
El modelo $\mathrm{AR}(p)$ en la ecuación ([-@eq-oarp]) puede ser escrito en notación de operador como
...
en el que $\sigma_{_a}^2 = 1$. La notación del operador para este modelo es
...
La @fig-ejar4 representa una realización de longitud $n = 200$ del proceso descrito en la ecuación ([-@eq-ejar4]), junto con las autocorrelaciones muestrales asociadas y la estimación de la densidad espectral Parzen, respectivamente.

```{r, results='hide'}
#| label: fig-ejar4  
#| fig-cap: "Realización del modelo AR(4)."
library(tswge)
x=gen.arma.wge(n=200, phi=c(0.1300,1.4414,-.0326,-.8865),
               sn=9310, plot=FALSE)
plotts.sample.wge(x)
```
:::
...
Se considera de nuevo el modelo $\mathrm{AR}(4)$ descrito en la ecuación ([-@eq-ejar4]). La ecuación característica asociada es
...
El error porcentual absoluto medio (MAPE) mide la precisión de la predicción como un porcentaje y se define generalmente de la siguiente manera. $$MAPE=\frac{100}{n}\sum_{t=1}^n \left| \frac{y_t-f_t}{y_t}\right|.$$ {#eq-MAPE}
