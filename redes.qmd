...
Una red neuronal artificial MLP se define formalmente como una tripla $<\mathscr D, \{f\}, \mathscr A>$, donde;
...
::: {style="text-align: justify"}
Generalizando la @eq-retrop y haciendo uso de la definición previa, se puede expresar la derivada parcial de la función de pérdida respecto a los pesos en la última capa como:
...
Al extender este proceso desde la @eq-18 hasta la @eq-21 para calcular $\frac{\partial \mathcal L}{\partial \mathbf b^L}$, se obtiene:
...
1.  Calcular el error $\mathbf e^L$ y $\delta^L$ en la última capa, como se muestra en la @eq-20 y @eq-21.
...
3.  Proceder a la actualización de pesos y sesgos. Para ello, se introduce la función de coste $\mathcal C$ aplicada a la pérdida $\mathcal L$ y se calculan las derivadas con respecto a los pesos y sesgos:
    ...
    Estas derivadas son equivalentes a las obtenidas en la @eq-21, @eq-22 y @eq-25. Luego, se aplica el algoritmo de descenso del gradiente para actualizar las matrices de pesos y sesgos:
    ...
    Note que en la @eq-26, las derivadas con respecto a $\mathbf w^L$ y $\mathbf b^L$, son las mismas que se dedujeron en la @eq-25, con estos últimos parámetros obtenidos, se puede aplicar el algoritmo del descenso del gradiente, para actualizar las matrices de pesos y sesgos, así:
