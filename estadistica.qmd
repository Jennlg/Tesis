...
x <- c(7.4,8.4,8.8,6.4,10.0,5.5,7.3,5.9,7.1,7.9,        
       7.6,8.8,6.1,7.2,6.6,7.0,5.3,7.9,8.1,7.6,        
       8.0,9.0,8.9,7.5,5.5,8.5,7.4,6.3,7.7,6.9) 
y <- c(79.8,82.0,76.1,62.7,98.2,43.0,76.5,61.4,78.5,88.7,        
       80.7,94.5,50.1,68.3,64.4,67.2,53.9,78.8,85.7,81.7,        
       84.2,87.8,94.1,78.2,62.4,85.1,77.8,67.6,70.2,73.6) 
cat('Media(x) =', mean(x)) 
cat('Desviación estándar(x) =', round(sd(x),2)) 
cat('Media(y) =', round(mean(y),2)) 
cat('Desviación estándar(y) =', round(sd(y),2))
```
...
```{r, message=FALSE, echo=FALSE,warning=FALSE}
par(mfrow = c(1, 2)) 
hist(x, main = '(a) Puntuaciones') 
hist(y, main = '(b) Calificación final')
```
